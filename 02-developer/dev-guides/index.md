...
If PostgreSQL is insufficient:
  ...
  - **Scale up SKU** (Azure allows online scaling)
  - **Re-index vectors** if dimension change required (downtime)
  - **Migrate to larger instance** using Azure migration tools
  - **Update connection strings** in Zep configuration
...
**Documentation Required**:
  ...
  - **Architecture Diagram**: Show encryption flow (Zep → PostgreSQL TDE → Key Vault)
...
**Testing Requirements**:
  ...
  - **Encryption Verification**: Verify memory data is encrypted in PostgreSQL
  - **Key Rotation Test**: Simulate key rotation without data loss
  - **Access Control Test**: Verify RBAC prevents unauthorized memory access
  - **Audit Logging Test**: Verify all memory operations are logged
