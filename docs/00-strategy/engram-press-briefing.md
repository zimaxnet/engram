...
## Technical Architecture: The Memory-First Stack
      ...
      Engram's memory subsystem comprises:
            ...
            **Fact Store**
            PostgreSQL + Zep
            Persistent, vector-indexed memory with temporal awareness
          
          
            **Knowledge Graph**
            `/api/v1/memory/graph`
            Entity-relationship visualization of facts and metadata
          
          
            **Ingestion API**
            `POST /memory/facts`
            Real-time fact injection with typed metadata
          
          
            **Retrieval**
            Semantic + keyword hybrid
            Context-aware recall with source attribution
...
## The Recursive Advantage
      ...
      By seeding Engram with its own development history, we've created an AI platform that:
        ...
        - **Learns from its own construction** — Architecture decisions inform future enhancements
        - **Onboards itself** — New team members (human or AI) inherit full project context
        - **Debugs with hindsight** — Past resolutions surface automatically during troubleshooting
        - **Evolves coherently** — Technical debt becomes visible, searchable, and addressable
...
### Engram represents a new category: Context Engineering Platforms
