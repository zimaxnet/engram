---
layout: default
title: Reference
nav_order: 9
has_children: true
---

# [Home](/) › Reference

# Reference Documentation

Technical reference documentation for Engram.

## Standard Operating Procedures (SOP)

Configuration and operational procedures:

### Authentication

- [Auth Master](sop/auth-master.md) - Master authentication guide
- [Auth Entra](sop/auth-entra.md) - Entra ID configuration
- [Auth API](api/auth-api.md) - API authentication
- [Auth ACA](sop/auth-aca.md) - Azure Container Apps auth
- [Auth AKS](sop/auth-aks.md) - Azure Kubernetes Service auth
- [Auth SWA](sop/auth-swa.md) - Static Web Apps auth
- [Auth Storage](sop/auth-storage.md) - Storage authentication
- [Auth Postgres](sop/auth-postgres.md) - PostgreSQL auth
- [Auth Zep](sop/auth-zep.md) - Zep memory auth
- [Auth Temporal](sop/auth-temporal.md) - Temporal auth
- [Auth KeyVault](sop/auth-keyvault.md) - Azure Key Vault
- [Auth CI/CD](sop/auth-cicd.md) - CI/CD authentication

### Configuration

- [Azure AI Configuration](sop/azure-ai-configuration.md)
- [Azure Foundry Chat SOP](sop/azure-foundry-chat-sop.md)
- [Chat Model Router](sop/chat-model-router-enterprise-deployment.md)
- [Google Login Setup](sop/google_login_setup.md)
- [Secrets Management](sop/secrets-management.md)
- [Stories Configuration](sop/stories-configuration.md)
- [Transcription Configuration](sop/transcription-configuration.md)
- [VoiceLive Configuration](sop/voicelive-configuration.md)

### Temporal

- [Temporal Azure Configuration](sop/temporal-azure-configuration.md)
- [Temporal Azure Research](sop/temporal-azure-research.md)
- [Temporal Worker Postmortem](sop/temporal-worker-postmortem.md)

### Other

- [Sage Nano Banana](sop/sage_nano_banana.md)
- [Zep Memory Client Compatibility](sop/zep-memory-client-compatibility.md)

## API Reference

- [Auth API](api/auth-api.md) - Authentication API

## Sessions

- [Temporal Integration Session](sessions/temporal-integration-session-20251224.md)

---

**Related**: [Operations](../operations/), [Deployment](../deployment/)
