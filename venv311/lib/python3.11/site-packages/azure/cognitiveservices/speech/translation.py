...
class SpeechTranslationConfig(SpeechConfig):
    ...
    - from endpoint: pass a subscription key or AzureKeyCredential or TokenCredential and an endpoint
    ...
    def __init__(self, subscription: OptionalStr = None, region: OptionalStr = None,
        ...
        if key_credential is not None and subscription is not None:
            raise ValueError('cannot specify both subscription key and key credential when constructing SpeechTranslationConfig.')

        if key_credential is not None and token_credential is not None:
            raise ValueError('cannot specify both key credential and token credential when constructing SpeechTranslationConfig.')
