...
class AzureAvatarService:
    ...
    async def create_session(
        self, session_id: str, agent_id: str = "elena"
    ) -> AvatarSession:
        """
    ...
    async def speak(
        self, session_id: str, text: str, visemes: Optional[list[dict]] = None
    ) -> AvatarSpeakResult:
        """
        ...
        if not session:
            return AvatarSpeakResult(
                success=False, duration_ms=0, error="Session not found"
            )

        if not session.is_active:
            return AvatarSpeakResult(
                success=False, duration_ms=0, error="Session is not active"
            )
        ...
        try:
            ...
            return AvatarSpeakResult(success=True, duration_ms=duration_ms)

        except Exception as e:
            logger.error(f"Avatar speak error: {e}")
            return AvatarSpeakResult(success=False, duration_ms=0, error=str(e))

    async def set_expression(self, session_id: str, expression: str) -> bool:
        """
    ...
    async def set_gesture(self, session_id: str, gesture: str) -> bool:
        """
...
def interpolate_visemes(
    visemes: list[dict], current_time_ms: int, smoothing: float = 0.3
) -> dict:
    """
...
async def create_avatar_session(
    session_id: str, agent_id: str = "elena"
) -> AvatarSession:
    """Create a new avatar session"""
...
async def avatar_speak(
    session_id: str, text: str, visemes: Optional[list[dict]] = None
) -> AvatarSpeakResult:
