...
async def get_temporal_client() -> Client:
    """Get or create the Temporal client with Azure Container Apps support"""
    global _client
    ...
    if _client is None:
        ...
        if use_tls:
            logger.info(f"Connecting to Temporal at {host}:{port} with TLS")
...
async def send_conversation_message(workflow_id: str, message: str) -> None:
    """
...
async def switch_conversation_agent(workflow_id: str, agent_id: str) -> None:
    """
...
async def send_approval_decision(
    ...
    await handle.signal(
        ...
        ApprovalSignal(approved=approved, feedback=feedback, approver_id=approver_id),
    )
