...
from backend.workflows.client import start_conversation, send_conversation_message
...
class BauService:
    ...
    def list_flows(self) -> List[BauFlow]:
        """List available BAU workflow templates."""
        return BAU_FLOWS

    async def list_artifacts(self, user_id: str, limit: int = 20, offset: int = 0) -> List[BauArtifact]:
        """
        List recent ingested artifacts with provenance metadata.
        """
        try:
            # Mock implementation - in production, query memory_client
            artifacts = []
            ...
            # Re-raise or return empty list depending on error handling strategy
            raise e

    async def start_flow(self, flow_id: str, user_id: str, tenant_id: str, initial_message: Optional[str] = None) -> StartBauFlowResponse:
        """Start a BAU workflow."""
        # Validate flow_id
        flow = next((f for f in BAU_FLOWS if f.id == flow_id), None)
        if not flow:
            raise ValueError(f"BAU flow '{flow_id}' not found")
        ...
        try:
            ...
            if initial_message:
                await send_conversation_message(workflow_id, initial_message)
