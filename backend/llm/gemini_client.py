...
Return ONLY valid JSON, no markdown code blocks or explanations."""
    ...
    async def generate_image(self, prompt: str) -> bytes:
        """
        Generate an image based on the prompt using Google's Imagen model.
        ...
        Returns:
            Raw image bytes (PNG)
        """
        logger.info(f"GeminiClient: Generating image for prompt: {prompt[:50]}...")
        
        # URL for Imagen 3 generation (using the beta API)
        # Note: Model name might change, using a likely stable endpoint or specific model
        # For now, trying imagen-3.0-generate-001 or equivalent available in v1beta
        url = f"{self.GEMINI_API_URL}/imagen-3.0-generate-001:predict?key={self.api_key}"
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            try:
                ...
                # Response format for Imagen usually involves 'predictions' -> 'bytesBase64Encoded'
                predictions = data.get("predictions", [])
                if predictions:
                    import base64
                    b64_data = predictions[0].get("bytesBase64Encoded")
                    if b64_data:
                        logger.info("GeminiClient: Image generated successfully")
                        return base64.b64decode(b64_data)
                
                logger.warning(f"GeminiClient: No image data in response: {data}")
                return b""
                
            except httpx.HTTPStatusError as e:
                logger.error(f"GeminiClient: Image generation HTTP error {e.response.status_code}: {e.response.text}")
                # Fallback to mock if API fails (so we don't break the flow completely during dev)
                logger.warning("Falling back to mock image generation due to API error")
                return await self._generate_mock_image(prompt)
            except Exception as e:
                logger.error(f"GeminiClient: Error generating image: {e}")
                return b""
